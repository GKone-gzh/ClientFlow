...
test("data access indexes preserve direct task-page user isolation", async () => {
  const db = await createDatabase();
  const projectA = "00000000-0000-4000-8000-00000000a201";
  const taskA = "00000000-0000-4000-8000-00000000a401";
  const taskB = "00000000-0000-4000-8000-00000000b401";
  ...
  try {
    await db.exec(`
      ...
      insert into public.projects (id, user_id, client_id, name)
      values
        ('${projectA}', '${userA}', '${clientA}', 'User A Project'),
        ('${projectB}', '${userB}', '${clientB}', 'User B Project');
      insert into public.tasks (id, user_id, project_id, title, created_at)
      values
        ('${taskA}', '${userA}', '${projectA}', 'User A Task', '2026-08-24T08:00:00Z'),
        ('${taskB}', '${userB}', '${projectB}', 'User B Task', '2026-08-24T08:00:00Z');
      set role authenticated;
      select set_config('request.jwt.claim.sub', '${userA}', false);
    `);

    const visibleTasks = await db.query(`
      select id
      from public.tasks
      where user_id = '${userA}'
      ...
      limit 51
    `);
    assert.deepEqual(visibleTasks.rows, [{ id: taskA }]);
    ...
    assert.match(
      ...
      /\(user_id, updated_at DESC, id DESC\)$/i,
    );
    assert.match(
      indexes.rows[1].indexdef,
      /\(user_id, created_at DESC, id DESC\)$/i,
    );
    assert.match(
      indexes.rows[2].indexdef,
      /\(user_id, project_id, sort_order, id\)$/i,
    );

    const privileges = await db.query(`
      select
        has_table_privilege('authenticated', 'public.tasks', 'SELECT') as can_select,
        has_table_privilege('authenticated', 'public.tasks', 'INSERT') as can_insert,
        has_table_privilege('authenticated', 'public.tasks', 'UPDATE') as can_update,
        has_table_privilege('authenticated', 'public.tasks', 'DELETE') as can_delete,
        has_table_privilege('authenticated', 'public.ai_extractions', 'UPDATE') as can_update_extractions
    `);
    assert.deepEqual(privileges.rows, [
      {
        can_select: true,
